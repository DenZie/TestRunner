...
// THIS WIDGET IS BASED ON DOJO/DIJIT 1.3.2 AND WILL NOT WORK WITH LATER VERSIONS
//
...
dojo.declare( "tmpdir.CheckBoxStoreModel", dijit.tree.ForestStoreModel, 
{
	...
	_setCheckboxState: function(/*dojo.data.Item*/ storeItem, /*Boolean*/ newState ) {
		...
		return stateChanged;	// the new state of the checkbox
	},
			...
			this.getChildren( parentItem, dojo.hitch( this,
				function( children ) {
					dojo.forEach( children, function(child) {
						this._setCheckboxState(child, newState);					
				...
				function(err) {
					console.error(_this, ": updating child checkboxes: ", err);
				} 
			);
		}
	},
	...
	_updateParentCheckbox: function(/*dojo.data.Item*/ storeItem, /*Boolean*/ newState ) {
		...
		//		The new state of the checkbox: true or false
		//
		var parentItem = this._getParentItem(storeItem);
		if( !parentItem ) { 
			return; 
		}
			...
			this.getChildren( parentItem, dojo.hitch( this,
				function(siblings) {
					...
					dojo.some( siblings, function(sibling) {
						siblState = this.store.getValue(sibling,this.checkboxIdent);
				...
				function(err) {
					console.error(_this, ": updating parent checkboxes: ", err);
				}
			);
	...
	_getParentItem: function(/*dojo.data.Item*/ storeItem ) {
		// summary:
		//		Get the parent of a dojo.data item.  
		// description:
		//		Get the parent of a dojo.data item. The '_reverseRefMap' entry of the item is
		...
		//
		//		NOTE: in case the item has multiple reference (ie, is referenced by multiple parents)
		//		only the FIRST reference is returned as the parent. (dijit.Tree 1.3.2 does not support
		//		multiple parents).
		//	storeItem:
		//		The dojo.data.item whos parent will be returned.
		//
		var parent = null;
		...
		if( storeItem != this.root ) {
			...
			for(itemId in references ) {
				parent = this.store._itemsByIdentity[itemId];
				break;
			}
			if (!parent) {
				parent = this.root;
		...
		return parent // parent of a dojo.data.item (dojo.data.item|null)
	},

 ...
 * runtime environment.
 * @author Daniel Le Berre, Elliott Wadejh
 */
public class ClassFinder
{
	...
	public final Map<URL, String> getClasspathLocations ()
	{
		...
		File file = null;
		String classpath = ClassFinder.class.getResource("DemoBase.class").getFile();
		int webinfindex = classpath.indexOf("WEB-INF");
		classpath = classpath.substring(0, webinfindex+15);
		file = new File (classpath);
		include (null, file, map);
		return map;
	}

	public final Object[] getcpkeys ()
	{

		 Object[] cs = getClasspathLocations ().keySet().toArray();
		 return cs;
	}
